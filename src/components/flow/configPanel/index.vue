...
export default {
  ...
  methods: {
    ...
    nodeClick(event) {
      switch (event) {
        case "edit":
          ...
          this.shapeType = this.nodeCell.shape;
    ...
    edgeAnimationDelete(isShow) {
      ...
      if (isShow) {
        //添加线选中动画
        const edgeAnimationSelect =
          this.container.querySelectorAll(".x6-edge-selected");
        edgeAnimationSelect[0].querySelectorAll("path")[1].style.animation =
          "ant-line 30s infinite linear";
        edgeAnimationSelect[0].querySelectorAll(
          "path"
        )[1].style.strokeDasharray = 5;
      } else {
        for (let i = 0, len = this.edgeAnimation.length; i < len; i = i + 1) {
          this.edgeAnimation[i].querySelectorAll("path")[1].style.animation =
            "";
          this.edgeAnimation[i].querySelectorAll(
            "path"
          )[1].style.strokeDasharray = 0;
        }
      }
    },
    //重置状态
